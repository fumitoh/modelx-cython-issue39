...
class CythonEmitter:
    ...
    def emit(self, module_name):
        ...
        lines += ["cpdef cnp.ndarray run(", f"    {np_args},", "    int threads=0,", "):"]
        ...
        lines += ["    cdef double[:,::1] out = out_arr", "    cdef Py_ssize_t i", "    if threads == 0:", "        threads = os.cpu_count() or 1", "        if threads > n:", "            threads = <int>n", "    if threads <= 1 or n <= 1:", "        with nogil:", "            for i in range(n):"]
        callargs=", ".join(f"m_{n}" for n in self.c.inputs)
        lines.append(f"                _run_one(i, {callargs}, out)")
        lines += ["    else:", "        for i in prange(n, nogil=True, schedule='static', num_threads=threads):", f"            _run_one(i, {callargs}, out)", "    return out_arr", ""]
        return "\n".join(lines)
