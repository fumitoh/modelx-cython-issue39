...
cdef object _calc_class_factor(rt):
...
cdef object fr_class_factor(rt):
    c = rt.cache[8]
    key = None
    if key in c:
        return c[key]
...
cdef object _calc_coi_rate(rt, t):
...
cdef object _calc_coi_rate_dp(rt):
...
cdef object _calc_coi_rate_guar(rt, t):
...
cdef object fr_coi_rate_guar(rt, t):
    c = rt.cache[12]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_coi_rate_scale(rt):
...
cdef object fr_coi_rate_scale(rt):
    c = rt.cache[13]
    key = None
    if key in c:
        return c[key]
...
cdef object _calc_corridor_factor(rt, t):
...
cdef object fr_corridor_factor(rt, t):
    c = rt.cache[14]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_crediting_rate_ann(rt, t):
...
cdef object fr_crediting_rate_ann(rt, t):
    c = rt.cache[15]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_cum_prem_init(rt):
...
cdef object fr_cum_prem_init(rt):
    c = rt.cache[17]
    key = None
    if key in c:
        return c[key]
...
cdef object _calc_cum_prem_pp(rt, t):
...
cdef object fr_cum_prem_pp(rt, t):
    c = rt.cache[18]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_grace_mth(rt, t):
...
cdef object _calc_guarantee_age(rt):
...
cdef object fr_guarantee_age(rt):
    c = rt.cache[26]
    key = None
    if key in c:
        return c[key]
...
cdef object _calc_inv_income_pp(rt, t):
...
cdef object fr_inv_income_pp(rt, t):
    c = rt.cache[29]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_inv_return_mth(rt, t):
...
cdef object fr_inv_return_mth(rt, t):
    c = rt.cache[30]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_is_guar_active(rt, t):
...
cdef object fr_is_guar_active(rt, t):
    c = rt.cache[31]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_is_guar_supported(rt, t):
...
cdef object fr_is_guar_supported(rt, t):
    c = rt.cache[32]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_is_lapsed(rt, t):
...
cdef object _calc_is_premium_mth(rt, t):
...
cdef object fr_is_premium_mth(rt, t):
    c = rt.cache[34]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_is_shortfall(rt, t):
...
cdef object fr_is_shortfall(rt, t):
    c = rt.cache[35]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_lapse_rate_dyn_mult(rt, t):
...
cdef object fr_lapse_rate_dyn_mult(rt, t):
    c = rt.cache[38]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_lapse_rate_guar_mult(rt):
...
cdef object fr_lapse_rate_guar_mult(rt):
    c = rt.cache[39]
    key = None
    if key in c:
        return c[key]
...
cdef object _calc_lapse_rate_pattern_mult(rt):
...
cdef object fr_lapse_rate_pattern_mult(rt):
    c = rt.cache[41]
    key = None
    if key in c:
        return c[key]
...
cdef object _calc_load_prem_rate(rt):
...
cdef object fr_load_prem_rate(rt):
    c = rt.cache[42]
    key = None
    if key in c:
        return c[key]
...
cdef object _calc_loan_bal_init(rt):
...
cdef object fr_loan_bal_init(rt):
    c = rt.cache[43]
    key = None
    if key in c:
        return c[key]
...
cdef object _calc_loan_bal_pp(rt, t):
...
cdef object fr_loan_bal_pp(rt, t):
    c = rt.cache[44]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_loan_cr_rate_mth(rt):
...
cdef object fr_loan_cr_rate_mth(rt):
    c = rt.cache[45]
    key = None
    if key in c:
        return c[key]
...
cdef object _calc_loan_rate_mth(rt):
...
cdef object fr_loan_rate_mth(rt):
    c = rt.cache[46]
    key = None
    if key in c:
        return c[key]
...
cdef object _calc_mort_improve_rate(rt, t):
...
cdef object fr_mort_improve_rate(rt, t):
    c = rt.cache[50]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_naar_factor(rt):
...
cdef object _calc_ncsv_pp(rt, t):
...
cdef object _calc_net_amt_at_risk(rt, t):
...
cdef object fr_net_amt_at_risk(rt, t):
    c = rt.cache[55]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_pols_lapse_grace(rt, t):
...
cdef object fr_pols_lapse_grace(rt, t):
    c = rt.cache[62]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_pols_rop(rt, t):
...
cdef object _calc_prem_persistency(rt, t):
...
cdef object fr_prem_persistency(rt, t):
    c = rt.cache[64]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_prem_to_sg_pp(rt, t):
...
cdef object fr_prem_to_sg_pp(rt, t):
    c = rt.cache[66]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_premium_freq(rt):
...
cdef object fr_premium_freq(rt):
    c = rt.cache[67]
    key = None
    if key in c:
        return c[key]
...
cdef object _calc_premium_pp_ann(rt):
...
cdef object fr_premium_pp_ann(rt):
    c = rt.cache[70]
    key = None
    if key in c:
        return c[key]
...
cdef object _calc_premium_type(rt):
...
cdef object fr_premium_type(rt):
    c = rt.cache[72]
    key = None
    if key in c:
        return c[key]
...
cdef object _calc_rider_charge_pp(rt, t):
...
cdef object fr_rider_charge_pp(rt, t):
    c = rt.cache[76]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_rop_anniversary(rt, t):
...
cdef object fr_rop_anniversary(rt, t):
    c = rt.cache[77]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_rop_elected(rt):
...
cdef object _calc_rop_rate(rt, t):
...
cdef object _calc_rop_ratio(rt, t):
...
cdef object _calc_sg_coi_pp(rt, t):
...
cdef object _calc_sg_coi_rate(rt, t):
...
cdef object fr_sg_coi_rate(rt, t):
    c = rt.cache[83]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_sg_inv_income_pp(rt, t):
...
cdef object fr_sg_inv_income_pp(rt, t):
    c = rt.cache[84]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_sg_maint_fee_pp(rt, t):
...
cdef object fr_sg_maint_fee_pp(rt, t):
    c = rt.cache[85]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_sg_naar_factor(rt):
...
cdef object fr_sg_naar_factor(rt):
    c = rt.cache[86]
    key = None
    if key in c:
        return c[key]
...
cdef object _calc_sg_net_amt_at_risk(rt, t):
...
cdef object fr_sg_net_amt_at_risk(rt, t):
    c = rt.cache[87]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_sg_net_pp(rt, t):
...
cdef object _calc_sg_pp_at(rt, t, timing):
...
cdef object fr_sg_pp_at(rt, t, timing):
    c = rt.cache[90]
    key = (t, timing)
    if key in c:
        return c[key]
...
cdef object _calc_sg_pp_init(rt):
...
cdef object _calc_sg_rate_mth(rt):
...
cdef object _calc_sum_assured_at(rt, t):
...
cdef object fr_sum_assured_at(rt, t):
    c = rt.cache[94]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_wd_fee_pp(rt, t):
