...
cdef object _calc_adj_pp_bef_anniv(rt, t):
...
cdef object fr_adj_pp_bef_anniv(rt, t):
    c = rt.cache[1]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_age_at_anniv(rt, t):
...
cdef object fr_age_at_anniv(rt, t):
    c = rt.cache[4]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_asset_charge_pp(rt, t):
...
cdef object fr_asset_charge_pp(rt, t):
    c = rt.cache[7]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_asset_charges(rt, t):
...
cdef object fr_asset_charges(rt, t):
    c = rt.cache[8]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_av_pp_at(rt, t, timing):
...
cdef object fr_av_pp_at(rt, t, timing):
    c = rt.cache[10]
    key = (t, timing)
    if key in c:
        return c[key]
...
cdef object _calc_av_pp_init(rt):
...
cdef object _calc_bb_pp_bef_anniv(rt, t):
...
cdef object fr_bb_pp_bef_anniv(rt, t):
    c = rt.cache[13]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_bonus_end(rt, t):
...
cdef object _calc_bonus_end_init(rt):
...
cdef object fr_bonus_end_init(rt):
    c = rt.cache[17]
    key = None
    if key in c:
        return c[key]
...
cdef object _calc_bonus_pp(rt, t):
...
cdef object _calc_charge_income(rt, t):
...
cdef object fr_charge_income(rt, t):
    c = rt.cache[20]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_charge_pp(rt, t):
...
cdef object _calc_charge_pp_due(rt, t):
...
cdef object fr_charge_pp_due(rt, t):
    c = rt.cache[22]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_charge_scale(rt, t):
...
cdef object fr_charge_scale(rt, t):
    c = rt.cache[23]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_claim_pp(rt, t, kind):
...
cdef object fr_claim_pp(rt, t, kind):
    c = rt.cache[24]
    key = (t, kind)
    if key in c:
        return c[key]
...
cdef object _calc_cv_pre_excess_pp(rt, t):
...
cdef object fr_cv_pre_excess_pp(rt, t):
    c = rt.cache[29]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_depleted_flag(rt, t):
...
cdef object fr_depleted_flag(rt, t):
    c = rt.cache[31]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_duration_mth_init(rt):
...
cdef object fr_duration_mth_init(rt):
    c = rt.cache[34]
    key = None
    if key in c:
        return c[key]
...
cdef object _calc_excess_factor(rt, t):
...
cdef object fr_excess_factor(rt, t):
    c = rt.cache[35]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_fee_glwb_pp_due(rt, t):
...
cdef object fr_fee_glwb_pp_due(rt, t):
    c = rt.cache[38]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_fee_gmdb_pp_due(rt, t):
...
cdef object fr_fee_gmdb_pp_due(rt, t):
    c = rt.cache[40]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_fee_rate_vix_clip(rt, prior, raw):
...
cdef object fr_fee_rate_vix_clip(rt, prior, raw):
    c = rt.cache[41]
    key = (prior, raw)
    if key in c:
        return c[key]
...
cdef object _calc_fee_rate_vix_raw(rt, phi_0, vix_sq_avg):
...
cdef object fr_fee_rate_vix_raw(rt, phi_0, vix_sq_avg):
    c = rt.cache[42]
...
cdef object _calc_fee_reset_rule(rt):
...
cdef object fr_fee_reset_rule(rt):
    c = rt.cache[43]
    key = None
    if key in c:
        return c[key]
...
cdef object _calc_forlife_flag(rt):
...
cdef object fr_forlife_flag(rt):
    c = rt.cache[46]
    key = None
    if key in c:
        return c[key]
...
cdef object _calc_free_wd_allow(rt, t):
...
cdef object fr_free_wd_allow(rt, t):
    c = rt.cache[47]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_free_wd_avail(rt, t):
...
cdef object fr_free_wd_avail(rt, t):
    c = rt.cache[48]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_free_wd_used_cum_pp(rt, t):
...
cdef object fr_free_wd_used_cum_pp(rt, t):
    c = rt.cache[49]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_fund_expense_rate(rt, i):
...
cdef object fr_fund_expense_rate(rt, i):
    c = rt.cache[50]
    key = i
    if key in c:
        return c[key]
...
cdef object _calc_gawa_pct_at_age(rt, a):
...
cdef object fr_gawa_pct_at_age(rt, a):
    c = rt.cache[52]
    key = a
    if key in c:
        return c[key]
...
cdef object _calc_gawa_pct_fixed(rt, t):
...
cdef object fr_gawa_pct_fixed(rt, t):
    c = rt.cache[53]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_gawa_pp(rt, t):
...
cdef object _calc_gawa_pp_at(rt, t, timing):
...
cdef object fr_gawa_pp_at(rt, t, timing):
    c = rt.cache[56]
    key = (t, timing)
    if key in c:
        return c[key]
...
cdef object _calc_glwb_payment_pp(rt, t):
...
cdef object fr_glwb_payment_pp(rt, t):
    c = rt.cache[59]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_glwb_payments(rt, t):
...
cdef object fr_glwb_payments(rt, t):
    c = rt.cache[60]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_gmdb_allow_pp(rt, t):
...
cdef object fr_gmdb_allow_pp(rt, t):
    c = rt.cache[61]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_gmdb_dfd_acc_pp(rt, t):
...
cdef object fr_gmdb_dfd_acc_pp(rt, t):
    c = rt.cache[62]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_gmdb_factor_acc(rt, t):
...
cdef object fr_gmdb_factor_acc(rt, t):
    c = rt.cache[63]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_gmdb_guarantee_pp(rt, t):
...
cdef object fr_gmdb_guarantee_pp(rt, t):
    c = rt.cache[64]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_gmdb_option(rt):
...
cdef object _calc_gmdb_wd_dfd_pp(rt, t):
...
cdef object fr_gmdb_wd_dfd_pp(rt, t):
    c = rt.cache[66]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_gmdb_wd_excess_pp(rt, t):
...
cdef object fr_gmdb_wd_excess_pp(rt, t):
    c = rt.cache[67]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_gmdb_wd_factor(rt, t):
...
cdef object fr_gmdb_wd_factor(rt, t):
    c = rt.cache[68]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_gwb_adj_year(rt):
...
cdef object fr_gwb_adj_year(rt):
    c = rt.cache[69]
    key = None
    if key in c:
        return c[key]
...
cdef object _calc_gwb_pp_aft_stepup(rt, t):
...
cdef object fr_gwb_pp_aft_stepup(rt, t):
    c = rt.cache[72]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_gwb_pp_at(rt, t, timing):
...
cdef object fr_gwb_pp_at(rt, t, timing):
    c = rt.cache[73]
    key = (t, timing)
    if key in c:
        return c[key]
...
cdef object _calc_has_wd_by(rt, t):
...
cdef object _calc_has_wd_init(rt):
...
cdef object _calc_inv_return_mth(rt, t, i):
...
cdef object fr_inv_return_mth(rt, t, i):
    c = rt.cache[78]
    key = (t, i)
    if key in c:
        return c[key]
...
cdef object _calc_is_anniv(rt, t):
...
cdef object _calc_is_first_wd(rt, t):
...
cdef object fr_is_first_wd(rt, t):
    c = rt.cache[80]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_is_gwb_adj_date(rt, t):
...
cdef object fr_is_gwb_adj_date(rt, t):
    c = rt.cache[81]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_is_quarterly_anniv(rt, t):
...
cdef object fr_is_quarterly_anniv(rt, t):
    c = rt.cache[83]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_is_stepup(rt, t):
...
cdef object _calc_is_wd_month(rt, t):
...
cdef object fr_is_wd_month(rt, t):
    c = rt.cache[85]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_is_wd_taken(rt, t):
...
cdef object fr_is_wd_taken(rt, t):
    c = rt.cache[86]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_is_wd_year(rt, t):
...
cdef object fr_is_wd_year(rt, t):
    c = rt.cache[87]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_lapse_dyn_mult(rt, t):
...
cdef object fr_lapse_dyn_mult(rt, t):
    c = rt.cache[89]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_lapse_itm_mult(rt, m):
...
cdef object fr_lapse_itm_mult(rt, m):
    c = rt.cache[90]
    key = m
    if key in c:
        return c[key]
...
cdef object _calc_lapse_wd_factor(rt, t):
...
cdef object fr_lapse_wd_factor(rt, t):
    c = rt.cache[94]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_maint_fee_pp_due(rt, t):
...
cdef object fr_maint_fee_pp_due(rt, t):
    c = rt.cache[96]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_moneyness_glwb(rt, t):
...
cdef object fr_moneyness_glwb(rt, t):
    c = rt.cache[99]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_moneyness_gmdb(rt, t):
...
cdef object fr_moneyness_gmdb(rt, t):
    c = rt.cache[100]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_phi_glwb(rt, t):
...
cdef object _calc_phi_glwb_vix(rt, k):
...
cdef object fr_phi_glwb_vix(rt, k):
    c = rt.cache[107]
    key = k
    if key in c:
        return c[key]
...
cdef object _calc_phi_gmdb(rt, t):
...
cdef object _calc_pols_decr(rt, t, kind):
...
cdef object fr_pols_decr(rt, t, kind):
    c = rt.cache[112]
    key = (t, kind)
    if key in c:
        return c[key]
...
cdef object _calc_prem_scheduled_pp(rt, t):
...
cdef object fr_prem_scheduled_pp(rt, t):
    c = rt.cache[118]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_prem_to_av_pp(rt, t):
...
cdef object fr_prem_to_av_pp(rt, t):
    c = rt.cache[119]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_premium_tax_rate(rt):
...
cdef object fr_premium_tax_rate(rt):
    c = rt.cache[122]
    key = None
    if key in c:
        return c[key]
...
cdef object _calc_rb_pp_at(rt, t, timing):
...
cdef object fr_rb_pp_at(rt, t, timing):
    c = rt.cache[127]
    key = (t, timing)
    if key in c:
        return c[key]
...
cdef object _calc_rb_prior_anniv_pp(rt, t):
...
cdef object fr_rb_prior_anniv_pp(rt, t):
    c = rt.cache[129]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_rollup_pct(rt):
...
cdef object _calc_rollup_rate(rt, t):
...
cdef object fr_rollup_rate(rt, t):
    c = rt.cache[131]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_rollup_rule(rt):
    """``fixed`` or ``cmt_linked``.
...
cdef object _calc_rp_reduction_pp(rt, t):
...
cdef object fr_rp_reduction_pp(rt, t):
    c = rt.cache[135]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_sa_pp_at(rt, t, i, timing):
...
cdef object fr_sa_pp_at(rt, t, i, timing):
    c = rt.cache[137]
    key = (t, i, timing)
    if key in c:
        return c[key]
...
cdef object _calc_scenario_rate(rt, t, name):
...
cdef object fr_scenario_rate(rt, t, name):
    c = rt.cache[139]
    key = (t, name)
    if key in c:
        return c[key]
...
cdef object _calc_stepup_base_pp(rt, t):
...
cdef object fr_stepup_base_pp(rt, t):
    c = rt.cache[141]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_stepup_basis(rt):
...
cdef object fr_stepup_basis(rt):
    c = rt.cache[142]
    key = None
    if key in c:
        return c[key]
...
cdef object _calc_sum_wd_pp(rt, t):
...
cdef object _calc_surr_benefit_pp(rt, t):
...
cdef object fr_surr_benefit_pp(rt, t):
    c = rt.cache[145]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_surr_charge_rate(rt, t):
...
cdef object fr_surr_charge_rate(rt, t):
    c = rt.cache[147]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_surr_chargeable_pp(rt, t):
...
cdef object fr_surr_chargeable_pp(rt, t):
    c = rt.cache[148]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_surr_free_pp(rt, t):
...
cdef object fr_surr_free_pp(rt, t):
    c = rt.cache[149]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_unit_growth(rt, t, i):
...
cdef object fr_unit_growth(rt, t, i):
    c = rt.cache[152]
    key = (t, i)
    if key in c:
        return c[key]
...
cdef object _calc_wd_excess_pp(rt, t):
...
cdef object fr_wd_excess_pp(rt, t):
    c = rt.cache[156]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_wd_exempt_pp(rt, t):
...
cdef object fr_wd_exempt_pp(rt, t):
    c = rt.cache[157]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_wd_free_pp(rt, t):
...
cdef object fr_wd_free_pp(rt, t):
    c = rt.cache[158]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_wd_glwb_pp(rt, t):
...
cdef object fr_wd_glwb_pp(rt, t):
    c = rt.cache[159]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_wd_intensity(rt):
...
cdef object fr_wd_intensity(rt):
    c = rt.cache[160]
    key = None
    if key in c:
        return c[key]
...
cdef object _calc_wd_limit_pp(rt, t):
...
cdef object fr_wd_limit_pp(rt, t):
    c = rt.cache[161]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_wd_pp_due(rt, t):
...
cdef object _calc_wd_scheduled_pp(rt, t):
...
cdef object fr_wd_scheduled_pp(rt, t):
    c = rt.cache[166]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_wd_start_age(rt):
...
cdef object fr_wd_start_age(rt):
    c = rt.cache[167]
    key = None
    if key in c:
        return c[key]
