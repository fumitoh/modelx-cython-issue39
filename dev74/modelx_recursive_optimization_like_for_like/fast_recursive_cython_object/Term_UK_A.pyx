...
cdef object _calc_age_at_entry(rt, life=1):
...
cdef object fr_age_at_entry(rt, life=1):
    c = rt.cache[1]
    key = life
    if key in c:
        return c[key]
...
cdef object _calc_annuity_certain_factor(rt, m):
...
cdef object fr_annuity_certain_factor(rt, m):
    c = rt.cache[2]
    key = m
    if key in c:
        return c[key]
...
cdef object _calc_benefit_pp(rt, t):
...
cdef object fr_benefit_pp(rt, t):
    c = rt.cache[4]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_benefit_sched(rt, k):
...
cdef object fr_benefit_sched(rt, k):
    c = rt.cache[5]
    key = k
    if key in c:
        return c[key]
...
cdef object _calc_claim_expenses(rt, t):
...
cdef object fr_claim_expenses(rt, t):
    c = rt.cache[6]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_comm_clawback(rt, t):
...
cdef object fr_comm_clawback(rt, t):
    c = rt.cache[8]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_comm_init_pp(rt):
...
cdef object fr_comm_init_pp(rt):
    c = rt.cache[9]
    key = None
    if key in c:
        return c[key]
...
cdef object _calc_duration_inforce(rt):
...
cdef object fr_duration_inforce(rt):
    c = rt.cache[12]
    key = None
    if key in c:
        return c[key]
...
cdef object _calc_fib_commute_pp(rt, t):
...
cdef object fr_fib_commute_pp(rt, t):
    c = rt.cache[14]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_fib_commute_rate(rt):
...
cdef object fr_fib_commute_rate(rt):
    c = rt.cache[15]
    key = None
    if key in c:
        return c[key]
...
cdef object _calc_fib_cum(rt, t):
...
cdef object _calc_idx_factor(rt, t):
...
cdef object fr_idx_factor(rt, t):
    c = rt.cache[18]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_idx_increase(rt):
...
cdef object fr_idx_increase(rt):
    c = rt.cache[19]
    key = None
    if key in c:
        return c[key]
...
cdef object _calc_idx_prem_factor(rt, t):
...
cdef object fr_idx_prem_factor(rt, t):
    c = rt.cache[20]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_indexation(rt):
...
cdef object _calc_lapse_cum(rt, t):
...
cdef object _calc_mort_rate_base(rt, t, life=1):
...
cdef object fr_mort_rate_base(rt, t, life=1):
    c = rt.cache[30]
    key = (t, life)
    if key in c:
        return c[key]
...
cdef object _calc_mort_rate_life(rt, t, life=1):
...
cdef object fr_mort_rate_life(rt, t, life=1):
    c = rt.cache[31]
    key = (t, life)
    if key in c:
        return c[key]
...
cdef object _calc_pols_payer(rt, t):
...
cdef object fr_pols_payer(rt, t):
    c = rt.cache[39]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_premium_mth_pp(rt):
...
cdef object fr_premium_mth_pp(rt):
    c = rt.cache[40]
    key = None
    if key in c:
        return c[key]
...
cdef object _calc_proj_start(rt):
...
cdef object _calc_rebroke_factor(rt, t):
...
cdef object fr_rebroke_factor(rt, t):
    c = rt.cache[45]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_sched_rate(rt):
...
cdef object _calc_sched_rate_mth(rt):
...
cdef object fr_sched_rate_mth(rt):
    c = rt.cache[47]
    key = None
    if key in c:
        return c[key]
...
cdef object _calc_sel_lapse_factor(rt, t):
...
cdef object fr_sel_lapse_factor(rt, t):
    c = rt.cache[48]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_select_factor(rt, t):
...
cdef object fr_select_factor(rt, t):
    c = rt.cache[49]
    key = t
    if key in c:
        return c[key]
...
cdef object _calc_sex(rt, life=1):
...
cdef object _calc_smoker(rt, life=1):
...
cdef object fr_smoker(rt, life=1):
    c = rt.cache[52]
    key = life
    if key in c:
        return c[key]
...
cdef object _calc_wop_waived_frac(rt, t):
...
cdef object fr_wop_waived_frac(rt, t):
    c = rt.cache[56]
    key = t
    if key in c:
        return c[key]
